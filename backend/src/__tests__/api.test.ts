...
describe('API Integration Tests', () => {
  ...
  describe('GET /vehicles', () => {
    ...
    it('should have correct content-type header', async () => {
      ...
      // Act & Assert
      await request(app).get('/vehicles').expect('Content-Type', /json/).expect(200)
    })
  })
  ...
  describe('POST /checks', () => {
    ...
    it('should create check and return 201', async () => {
      // Act & Assert
      const response = await request(app).post('/checks').send(validCheckBody).expect(201)
    ...
    it('should set hasIssue to true when any item fails', async () => {
      ...
      // Act & Assert
      const response = await request(app).post('/checks').send(bodyWithFailure).expect(201)
    ...
    it('should return 400 for invalid vehicleId', async () => {
      ...
      // Act & Assert
      const response = await request(app).post('/checks').send(invalidBody).expect(400)
    ...
    it('should accept check without optional note', async () => {
      ...
      // Act & Assert
      const response = await request(app).post('/checks').send(bodyWithoutNote).expect(201)
  ...
  describe('GET /checks', () => {
    ...
    it('should return checks filtered by vehicleId', async () => {
      // Act & Assert
      const response = await request(app).get('/checks').query({ vehicleId: 'VH001' }).expect(200)

      expect(response.body).toHaveLength(2)
      expect(response.body.every((c: Check) => c.vehicleId === 'VH001')).toBe(true)
    })

    it('should filter by hasIssue=true', async () => {
      // Act & Assert
      const response = await request(app).get('/checks').query({ vehicleId: 'VH001', hasIssue: 'true' }).expect(200)
    ...
    it('should filter by hasIssue=false', async () => {
      // Act & Assert
      const response = await request(app).get('/checks').query({ vehicleId: 'VH001', hasIssue: 'false' }).expect(200)
    ...
    it('should return empty array when no checks match', async () => {
      // Act & Assert
      const response = await request(app).get('/checks').query({ vehicleId: 'VH999' }).expect(200)
  ...
  describe('DELETE /checks/:id', () => {
    ...
    it('should return 404 when check does not exist', async () => {
      // Act & Assert
      const response = await request(app).delete('/checks/NONEXISTENT').expect(404)
